...
mod tests {
    ...
    #[test]
    fn packed_iszfilter_candidates_diverge_at_height_two() {
        fn xs(s: &mut u64) -> u64 {
            let mut x = *s;
            x ^= x << 13;
            x ^= x >> 7;
        ...
        fn scalar_filtered(b: &Board, p: Piece, force: bool) -> Vec<u16> {
            let mut moves = MoveBuffer::new();
            generate(b, &mut moves, p, force);
            let reach = crate::pathfinder::reachable_locks(b, p, force);
            moves.retain(|m| b.legal_lock_placement(m) && reach.move_reachable(m));
        ...
        fn packed_filtered(b: &Board, p: Piece, force: bool) -> Vec<u16> {
            let rows30: &[u16; crate::reach_packed::PH] =
                b.rows[..crate::reach_packed::PH].try_into().unwrap();
            let mut packed = MoveBuffer::new();
            crate::reach_packed::generate_packed_with_force(rows30, p, force, &mut packed);
            let mut pset: Vec<u16> = packed.as_slice().iter().map(|m| m.raw()).collect();
            pset.sort_unstable();
            pset.dedup();
            let mut moves = MoveBuffer::new();
            generate(b, &mut moves, p, force);
            moves.retain(|m| b.legal_lock_placement(m) && pset.binary_search(&m.raw()).is_ok());
            moves.as_slice().iter().map(|m| m.raw()).collect()
        }

        let pieces = [Piece::I, Piece::S, Piece::Z];
        let mut found = [false; 3];
        let mut specimens = [String::new(), String::new(), String::new()];
        let mut st = 0x15A1_DA7A_C0DE_2026u64;
        for _ in 0..5000 {
            let mut rows = vec![0u16; 2];
            for r in rows.iter_mut() {
                *r = (xs(&mut st) as u16) & 0x3FF;
            }
            if let Some(l) = rows.last_mut() {
            ...
            for (pi, &p) in pieces.iter().enumerate() {
                ...
                for force in [false, true] {
                    let scalar = scalar_filtered(&b, p, force);
                    let hybrid = packed_filtered(&b, p, force);
                    if hybrid != scalar {
                        found[pi] = true;
                        specimens[pi] = format!(
                            "p={p:?} force={force} rows={rows:?} scalar_len={} packed_len={}",
                            scalar.len(),
                            hybrid.len()
                        );
                        break;
        ...
        for (i, &p) in pieces.iter().enumerate() {
            assert!(
                found[i],
                "expected a height-2 packed-filter divergence for {p:?}"
            );
            eprintln!("height-2 packed-filter divergence: {}", specimens[i]);
        }
    }
